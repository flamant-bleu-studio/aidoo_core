...
<div class="content_titre">
	<h1>{t}FavIcon Manager{/t}</h1>
	<div>{t}FavIcon Manager{/t}</div>
</div>	
	
<div class="zone">
	<div class="zone_titre">
		<h2>{t}Active Skin{/t} : {$activeSkin}<span class="helper"></span></h2>
		<div>{t}Active Sub-Domain{/t} : {$activeMulti}</div>
	</div>
	
	{if $favicon != null}
		<div>Your favIcon:<img src="{$favicon}" /> </div>
	{else}
		<div>No Favicon for the active skin </div>
	{/if}
</div>
