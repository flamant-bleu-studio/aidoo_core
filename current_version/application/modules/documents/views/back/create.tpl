...
<div class="content_titre">
	<h1>{t}Create a new document{/t}</h1>
	<div>{t}Creation of a new document{/t}</div>
</div>

<div class="zone">
	<div class="zone_titre">
		<h2>{t}Select a type{/t} <span class="helper"></span></h2>
		<div>{t}Select a type for your new document{/t}</div>
	</div>
		
	{if $types}
		<div class="choixDoc">		
		{foreach from=$types item=item}					
			<button id="documents_{$item["type"]}" class="typeChoice" data-choice="{$item["type"]}" data-desc="{$item["description"]}"></button>
		{/foreach}
		
			<p id="doc_types_desc" class="help-block">
				{t}Select a type for your new document{/t}
			</p>
