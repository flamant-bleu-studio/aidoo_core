...
<form action='{$formCampaign->getAction()}' method="post" id="{$formCampaign->getId()}"> 
	<div class="zone">
		...
		<div class="left">{$formCampaign->title}</div>
	...
	<div class="zone">
		...
		<div id="pub_manage">
			{if $decoded && $decoded|@count > 0}
				{foreach from=$decoded key=k item=i}
					<div class='pub'>
						<a href='#' class='pub_delete'></a>
						<img class='pubs_pub absolute' width='100' height='80' rel='{$k}' src='{$i.image_path_thumb}'/>
					</div>
				{/foreach}
			{/if}
		</div>
		...
		<div id="form_galerie">
			...
			<div class="center">
				<div id="form_line">
					<div class="form_elem">
						<a id="pub_del" class="btn btn_rouge">
