...
class Bloc_Articles_AdminForm extends CMS_Bloc_ParentForm
{
	...
	public function init()
	{
		...
		$item = new Zend_Form_Element_Checkbox("showDate");
		$item->setLabel(_t("Date articles"));
		$item->setDescription(_t("Check to active"));
		$this->addElement($item);
		
		$item = new Zend_Form_Element_Text("dateFormat");
		$item->setLabel(_t("Format"));
		$item->setDescription(_t("Date format ( ex : http://bit.ly/5nzlUX )"));
		$this->addElement($item);
		
		$item = new Zend_Form_Element_Text("truncateText");
		$item->setLabel(_t("Truncate text"));
		$item->setDescription(_t("Number of letters from wich truncate text (0 = no truncate)"));
		$this->addElement($item);
		...
		$item->addMultiOption("all", "TOUTES");
		
		foreach($categories as $cat) {
			$item->addMultiOption($cat->id_categorie, $cat->title);
		}
