 ...
 * @version    $Id: Zip.php 387 2010-10-04 11:11:49Z llemoullec $
 */

/**
 * @see Zend_Filter_Compress_CompressAbstract
 */
require_once 'CMS/Filter/Compress/CompressAbstract.php';

class CMS_Filter_Compress_Zip extends CMS_Filter_Compress_CompressAbstract
{
    /**
     * Compression Options
     * array(
    ...
    {
        $zip = new ZipArchive();
        $res = $zip->open($this->getArchive(), ZipArchive::CREATE );
            ...
            if (is_dir($content)) {
                ...
                while (!empty($stack)) {
                    ...
                    foreach ($files as $file) {
                        $res = $zip->addFile($current . $file, $local . $file);
                        if ($res !== true) {
                            require_once 'Zend/Filter/Exception.php';
                            throw new Zend_Filter_Exception($this->_errorString($res));
                        }
